...
public class OrderOutboxScheduler implements OutboxScheduler {
    ...
    public void processOutboxMessage() {
        ...
        if(orderOutboxMessageResponse.isPresent() && orderOutboxMessageResponse.get().size() > 0) {
            ...
            log.info("Recieved  {} OrderOutboxMessages with ids {}, sending to kafka!", outboxMessages.size(),
                    outboxMessages.stream().map(OrderOutboxMessage::getId)
                            .map(UUID::toString).collect(Collectors.joining(",")));
            outboxMessages.forEach(outboxMessage ->
                    paymentResponseMessagePublisher.publish(outboxMessage, orderOutboxHelper::updateOutboxStatus));
